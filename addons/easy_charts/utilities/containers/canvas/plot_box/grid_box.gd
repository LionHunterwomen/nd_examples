...
func _draw_vertical_grid() -> void:
	...
	draw_line(box.position - Vector2(10, 0), Vector2(box.position.x, box.end.y) - Vector2(10, 0), get_parent().chart_properties.colors.grid, 3)
...
func _draw_horizontal_grid() -> void:
	...
	for _y in get_parent().chart_properties.y_scale + 1:
		...
		if _y == get_parent().chart_properties.y_scale:
			draw_line(left, right, (get_parent().chart_properties.colors.grid as Color).lightened(0.5) * 2.0, 3)
		else:
...
func _get_vertical_tick_label_pos(base_position: Vector2, text: String) -> Vector2:
	return  base_position + Vector2(
		...
		ThemeDB.fallback_font_size + (get_parent().chart_properties.x_tick_size if get_parent().chart_properties.draw_vertical_grid else 0.0)
	)
