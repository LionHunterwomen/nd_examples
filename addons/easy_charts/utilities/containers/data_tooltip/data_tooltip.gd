...
func update_position(position: Vector2) -> void:
	self.position.x = position.x - get_rect().size.x / 2.0
	self.position.y = get_parent().get_box().position.y + 10.0
...
func _draw() -> void:
	draw_dashed_line(get_rect().size / 2.0, Vector2(get_rect().size.x / 2.0, get_parent().get_box().size.y), Color.YELLOW)
